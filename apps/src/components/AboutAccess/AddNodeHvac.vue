<template>
  ...
  >
    <el-form ref="formRef" :model="form" label-width="120px" :rules="rules">
      ...
      <el-form-item label="设备标识" prop="selfId" required>
        <el-input-number v-model="form.selfId" :min="1" placeholder="请输入设备标识" style="width: 100%" />
      </el-form-item>

      <el-form-item label="RS485网关" prop="rs485GatewayId" required>
        <el-select v-model="form.rs485GatewayId" placeholder="请选择RS485网关" style="width: 100%">
          <el-option
...
import { DeviceTypeName, useDeviceStore } from "@/stores/device";
...
const rules = {
  ...
  rs485GatewayId: [{ required: true, message: "请选择RS485网关", trigger: "change" }],
};
...
watch(
  ...
  (newLabId) => {
    if (props.filterGatewayByLab && newLabId && form.rs485GatewayId) {
      ...
      const isValid = validGateways.some((g) => g.value === form.rs485GatewayId);
      if (!isValid) {
