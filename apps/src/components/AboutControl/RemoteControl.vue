...
const deviceOptions = computed(() => {
  return props.selectedRows.map((row) => ({
    value: row.id,
    // 用 getLabName 转换 labId 为名称
    label: `${row.airCond} (${getLabName(row.labId)})`,
    rawData: row,
  }));
});
...
const buildCommand = (device) => {
  ...
  return {
    ...
    args: [
      ...
      selfId, // args[1]: selfId
      valueMaps.power[form.power], // args[2]: 开关 (0=关, 1=开)
      valueMaps.mode[form.mode], // args[3]: 模式 (1=制热, 2=制冷, 4=送风, 8=除湿)
      form.temperature, // args[4]: 温度
      valueMaps.fanSpeed[form.fanSpeed], // args[5]: 风速 (0=自动, 1=低速, 2=中速, 3=高速)
    ],
  };
};
...
const handleConfirm = async () => {
  ...
  try {
    ...
    const promises = form.selectedDeviceIds.map((deviceId) => {
      const device = props.selectedRows.find(
        (row) => (row.id || row.deviceId || row.airCond) === deviceId,
      );
