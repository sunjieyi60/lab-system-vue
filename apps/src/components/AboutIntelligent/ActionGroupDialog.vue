          ...
          <el-card shadow="never">
            ...
            <div class="device-select-row">
              <div class="device-select-item">
                ...
                <el-select
                  ...
                  placeholder="选择设备类型"
                  style="width: 100%"
                  @change="handleDeviceTypeChange(action)"
                >
              ...
              <div class="device-select-item device-name-item">
                ...
                <el-select
                  ...
                  :placeholder="props.laboratoryId ? '选择设备' : '请先选择实验室'"
                  style="width: 100%"
                  filterable
                ...
                <div v-if="!props.laboratoryId" class="hint-text" style="color: #f56c6c; margin-top: 4px;">
            ...
            <div class="control-section">
              ...
              <template v-if="action.deviceType === 'CircuitBreak'">
                ...
                <el-form-item>
                  <el-radio-group v-model="action.commandLine" size="large">
...
const buildCommandArgs = (action, device) => {
  switch (action.deviceType) {
    case "AirCondition":
      if (action.commandLine === "ENHANCE_CONTROL_AIR_CONDITION") {
        return [
          device.address ?? 31,
          device.selfId ?? 1,
          action.params?.power ?? 1,
          action.params?.mode ?? 2,
          action.params?.temperature ?? 24,
          action.params?.fanSpeed ?? 0,
        ];
      }
      break;
    case "CircuitBreak":
    ...
    case "Access":
      return [device.address ?? 0, device.selfId ?? 1];
    case "Light":
      return [device.address ?? 0];
  }
  return [];
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      const actions = (newVal.actions || []).map((action) => ({
        ...action,
        params: action.params || createDefaultParams(),
      }));
      Object.assign(form, {
        id: newVal.id || "",
        conditionGroupId: newVal.conditionGroupId
          ? String(newVal.conditionGroupId)
          : null,
        actions: actions,
      });
    } else {
