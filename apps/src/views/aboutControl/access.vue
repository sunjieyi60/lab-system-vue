  ...
  <div class="door-control-page">
    <div class="main-content">
      ...
      <div class="filter-row first-row">
        ...
        <div class="stat">
          <span>{{ tableData.length }}扇房门</span>
        </div>
 ...
 <div class="button-group right-buttons">
      ...
      <div class="table-box">
        ...
        >
          ...
          <el-table-column prop="deviceName" label="房门" align="center" />
          <el-table-column prop="doorStatus" label="房门状态" align="center">
            <template #default="{ row }">
              <el-tag
                :type="row.doorStatus === '开' ? 'success' : 'info'"
                size="small"
              >
                {{ row.doorStatus }}
              </el-tag>
          ...
          <el-table-column prop="lockStatus" label="门锁状态" align="center">
            <template #default="{ row }">
              <el-tag
                :type="row.lockStatus === '长开' ? 'success' : 'danger'"
                size="small"
              >
                {{ row.lockStatus }}
              </el-tag>
          ...
          <el-table-column prop="delayTime" label="延时时间" align="center" />
...
import AddNode from "@/components/AboutAccess/AddNodeHvac.vue";
import RemoteControl from "@/components/AboutAccess/RemoteControl.vue";
import DelaySetting from "@/components/AboutAccess/DelaySetting.vue";
import IntelligentControl from "@/components/AboutAccess/IntelligentControl.vue";
...
const tableData = computed(() => {
  ...
  console.log('【调试】原始数据条数:', data.length);
  console.log('【调试】选中楼栋:', building.value, '选中单位:', unit.value);

  // 如果选择了楼栋，按楼栋筛选
  if (building.value && building.value !== 'all') {
    // 获取该楼栋下的所有实验室ID（转为字符串比较）
    const labsInBuilding = laboratoryList.value.filter(lab => String(lab.belongToBuilding) === String(building.value));
    console.log('【调试】该楼栋下的实验室:', labsInBuilding);
    const labIdsInBuilding = labsInBuilding.map(lab => String(lab.id));
    console.log('【调试】该楼栋下的实验室IDs:', labIdsInBuilding);
    data = data.filter((item) => labIdsInBuilding.includes(String(item.labId)));
    console.log('【调试】楼栋筛选后数据条数:', data.length);
  }

  // 如果选择了单位，按单位筛选
  if (unit.value && unit.value !== 'all') {
    // 获取该单位下的所有实验室ID（转为字符串比较）
    const labsInUnit = laboratoryList.value.filter(lab => {
      const depts = lab.belongToDepts || [];
      return depts.map(String).includes(String(unit.value));
    });
    console.log('【调试】该单位下的实验室:', labsInUnit);
    const labIdsInUnit = labsInUnit.map(lab => String(lab.id));
    console.log('【调试】该单位下的实验室IDs:', labIdsInUnit);
    data = data.filter((item) => labIdsInUnit.includes(String(item.labId)));
    console.log('【调试】单位筛选后数据条数:', data.length);
  }

  // 如果选择了具体实验室，按实验室ID筛选
  if (labNo.value !== "all") {
    data = data.filter((item) => String(item.labId) === String(labNo.value));
    console.log('【调试】实验室筛选后数据条数:', data.length);
  }
...
const loadAllDeviceData = async () => {
  ...
  try {
    ...
    console.log("【检查】所有门禁数据:", deviceStore.getAccessTableData);
  } catch (error) {
