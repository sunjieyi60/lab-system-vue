...
const resetForm = () => {
  ...
  Object.assign(form.timeRule, defaultForm.timeRule);
  form.dataGroup = JSON.parse(JSON.stringify(defaultForm.dataGroup));
  form.conditionGroups = JSON.parse(
    JSON.stringify(defaultForm.conditionGroups),
  );
  form.actionGroups = JSON.parse(JSON.stringify(defaultForm.actionGroups));
  form.alarmGroup = JSON.parse(JSON.stringify(defaultForm.alarmGroup));
  Object.assign(form.watchDog, defaultForm.watchDog);
};
    ...
    if (visible) {
      ...
      nextTick(() => {
        if (props.editData) {
          ...
          Object.assign(
            form.timeRule,
            JSON.parse(JSON.stringify(props.editData.timeRule || {})),
          );
          form.dataGroup = JSON.parse(
            JSON.stringify(props.editData.dataGroup || []),
          );
          form.conditionGroups = JSON.parse(
            JSON.stringify(props.editData.conditionGroups || []),
          );
          form.actionGroups = JSON.parse(
            JSON.stringify(props.editData.actionGroups || []),
          );
          form.alarmGroup = JSON.parse(
            JSON.stringify(props.editData.alarmGroup || []),
          );
          Object.assign(
            form.watchDog,
            JSON.parse(JSON.stringify(props.editData.watchDog || {})),
          );
        } else {
          resetForm();
        }
      ...
      conditionGroups: form.conditionGroups.map((group) => ({
        ...
        conditions: (group.conditions || []).map((cond) => {
          ...
          const isNumeric = [
            ...
            "humidity",
            "light",
          ].includes(cond.property);
          const value = isNumeric ? parseInt(cond.value) : cond.value;
          return {
            ...
            expr: dataGroupId
              ? `#{${dataGroupId}}.${cond.property} ${cond.operator} ${value}`
              : cond.expr,
            desc:
              cond.desc ||
              cond.displayText ||
              `${cond.property} ${cond.operator} ${value}`,
          };
        }),
      })),
