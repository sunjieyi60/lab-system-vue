...
<template>
    ...
    <div class="main-content">
      ...
      <div class="table-box" v-loading="isLoading" element-loading-text="加载中...">
        ...
        >
          ...
          <el-table-column prop="deviceName" label="电灯开关" align="center" min-width="120" show-overflow-tooltip />
          <el-table-column prop="isOpen" label="开关状态" align="center" min-width="100">
            <template #default="{ row }">
              <el-tag
  ...
  <!-- 远程控制弹窗 - 使用 LightControl 组件 -->
  <el-dialog
    v-model="showRemote"
    :title="currentSelectedRows.length > 1 ? `批量控制 (${currentSelectedRows.length}台设备)` : '照明远程控制'"
    width="480px"
  ...
  >
    ...
    <div v-else-if="currentSelectedRows.length > 1" class="dialog-content">
      <LightControl
        :device="selectedLightDevices"
        v-bind="lightControlConfig"
...
// 将选中的行数据转换为 Light 类型设备对象（单设备）
const selectedLightDevice = computed(() => {
  if (currentSelectedRows.value.length === 0) return null;
  const row = currentSelectedRows.value[0];
  return {
    id: row.id,
    deviceName: row.deviceName || '照明设备',
    deviceType: 'Light',
...
// 将选中的行数据转换为 Light 类型设备对象列表（批量）
const selectedLightDevices = computed(() => {
...
const handleRemoteControl = () => {
  ...
  // 保存当前选中的行数据
  const rawData = deviceStore.getLightTableData;
...
const handleDialogOpen = () => {
  nextTick(() => {
    if (lightControlRef.value && currentSelectedRows.value.length === 1) {
      const device = currentSelectedRows.value[0];
      // 根据设备状态设置灯光开关状态
      lightControlRef.value.setLightState(device.isOpen);
    }
  });
};
...
const handleControlExecute = async (tasks, callback) => {
  console.log('【LightControl】执行控制命令:', tasks);
...
const handleBatchControlExecute = async (tasks, callback) => {
  console.log('【LightControl】批量执行控制命令:', tasks);
