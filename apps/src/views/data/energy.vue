          ...
          >
            ...
            >
              <template #default="{ row }">
                <span style="color: #409eff">{{ row.energyKwh }}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="proportion"
...
// 获取默认时间范围（开始时间为当月1号，结束时间为当天）
const getDefaultTimeRange = () => {
  const now = new Date();
  const year = now.getFullYear();
  const month = now.getMonth();
  const startOfMonth = new Date(year, month, 1, 0, 0);
  const endOfToday = new Date(year, month, now.getDate(), 23, 59);
  ...
  return [format(startOfMonth), format(endOfToday)];
};
...
const initChart = () => {
  ...
  const option = {
    tooltip: {
      trigger: "item",
      formatter: (params) => {
        return `${params.name}: ${params.value} Kwh (${params.data.proportion}%)`;
      },
    },
    legend: {
      orient: "vertical",
