  ...
  >
    <el-form ref="formRef" :model="form" label-width="100px" :rules="rules">
      ...
      <el-form-item label="学年学期" prop="name" v-else>
        <el-select v-model="form.name" placeholder="请选择学年学期" style="width: 100%">
          <el-option
...
const dialogTitle = computed(() => {
  if (mode.value === "batch") return "修改学期";
...
/* 生成学年学期选项列表 从 2022-2023 第1学期 到 当前年份的下一年第2学期 */
const termOptions = computed(() => {
  const options = [];
  const currentYear = new Date().getFullYear();
  const endYear = currentYear + 1; // 下一年
  
  for (let year = 2022; year <= endYear; year++) {
    const nextYear = year + 1;
    options.push(`${year}-${nextYear} 第1学期`);
    options.push(`${year}-${nextYear} 第2学期`);
  }
