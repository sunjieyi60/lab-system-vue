...
import { getCourseSchedule } from "@/api/edu";
import LabRoomSchedule from "@/views/edu/components/LabRoomSchedule.vue";
...
// 计算属性：楼栋列表（从实验室数据中推断楼栋与单位的关联关系）
const buildingList = computed(() => {
  // 从实验室数据中提取楼栋与单位的关联关系
  const buildingDeptMap = new Map();
  laboratoryList.value.forEach((lab) => {
  ...
  // 从 userStore 获取楼栋基本信息
  const storeBuildings = userStore.getBuildingList || [];

  // 为楼栋补充 deptIds
  return storeBuildings.map((building) => {
...
const filteredLaboratoryList = computed(() => {
  return laboratoryList.value.filter((lab) => {
    ...
    if (selectedBuildings.value.length > 0) {
      const labBuildingId =
        lab.buildingId || lab.building?.id || lab.belongToBuilding;
      if (!selectedBuildings.value.includes(labBuildingId)) {
        return false;
      }
    }
    // 部门筛选 - 兼容 belongToDepts 数组
    if (selectedDepts.value.length > 0) {
      // 兼容多种字段名
      const labDeptIds = lab.belongToDepts || lab.belongToDeptIds || [];
      if (labDeptIds.length > 0) {
        // 如果实验室有单位数组，检查是否有交集
        const hasMatch = labDeptIds.some((deptId) =>
          selectedDepts.value.includes(deptId),
        );
        if (!hasMatch) {
          return false;
        }
...
const filteredRoomList = computed(() => {
  ...
  return laboratoryList.value.filter((lab) =>
    selectedLabs.value.includes(lab.id),
  );
});
...
onMounted(async () => {
  ...
  if (laboratoryList.value.length > 0) {
    // 全选部门 - 兼容 belongToDepts 数组
    const allDeptIds = [
      ...new Set(
        laboratoryList.value.flatMap((lab) => {
          const labDeptIds = lab.belongToDepts || lab.belongToDeptIds || [];
          if (labDeptIds.length > 0) {
            return labDeptIds;
          }
          return [lab.deptId || lab.dept?.id || lab.belongToDeptId].filter(
            Boolean,
          );
        }),
      ),
    ...
    const allBuildingIds = [
      ...new Set(
        laboratoryList.value
          .map(
            (lab) => lab.buildingId || lab.building?.id || lab.belongToBuilding,
          )
          .filter(Boolean),
      ),
    ];
